...
- **The Duolingo Hebrew tree, rebuilt — and then carried past where it stops** —
  ...
  **Word families** sit in Practice, for the thing that separates two thousand
  ...
  under **You → Settings**.
  **Word drill** is the one drill that takes a word *out* of its sentence: ten
  words from the vocabulary the lessons have taught — five to thirty, set under
  **You → Settings** — each asked once, in one of three ways dealt out evenly
  across the session: the word is read out and you tap the picture of it among
  four; or a picture and four words; or a picture and a box to write the word
  in. Only the first kind has a voice; the other two are for reading the word
  yourself and then producing it. A word with no picture is asked the same
  ways with words instead.
