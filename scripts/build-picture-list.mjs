...
for (const f of files) {
  ...
  for (const [en, prompt] of doc.items || []) items.push({ en, prompt, from: f });
}
...
for (const it of items) {
  ...
  if (seen.has(key)) { problems.push(`${it.from}: "${it.en}" files under "${key}", already listed in ${seen.get(key)}`); continue; }
  seen.set(key, it.from);
...
md.push("");
md.push(`${wanted.length} of ${rows.length} words still want a picture; ${done} are drawn and in the app. ${replacing} of the ones left already show a scraped photograph, which the new image replaces.`);
md.push("");
...
md.push("2. Save each image under the **file** name given, as a 256×256 WebP, into `public/duo/img/`. Where the row is marked ↻ the file is already there and is overwritten, which is the point: the scraped photograph goes.");
...
for (let i = 0; i < wanted.length; i += BATCH) {
  ...
  md.push("|---|---|---|---|---|---|---|");
  batch.forEach((r, j) => {
    md.push(`| ${i + j + 1}${r.replaces ? " ↻" : ""} | ${r.en} | ${r.he} | ${r.unit} | \`${r.key}\` | \`${r.file}.webp\` | ${r.prompt} |`);
  });
  md.push("");
...
console.log(`${rows.length} words listed across ${files.length} files: ${done} drawn, ${wanted.length} wanted (${replacing} replacing a photograph)`);
