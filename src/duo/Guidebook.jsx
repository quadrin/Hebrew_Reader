            ...
            <div className="d-card" key={i}>
              <div className="d-row" style={{ alignItems: "flex-start" }}>
                <button className="d-icon-btn" style={{ color: "var(--d-blue)", borderColor: "var(--d-blue)" }}
                  onClick={() => playPhrase(p.he, p.audio)} aria-label={`Play ${p.he}`}>
                  <Volume2 size={18} />
                </button>
                <div style={{ flex: 1 }}>
                  <div className="d-prompt-he" dir="rtl" lang="he" style={{ fontSize: 24 }}>
                    {p.he.split(/(\s+)/).map((w, k) => {
                      ...
                      return (
                        <span key={k} style={{ position: "relative", display: "inline-block" }}>
                          <button type="button" className="d-hint" onClick={() => setHint(hint === `${i}-${k}` ? null : `${i}-${k}`)}>{w}</button>
                          {hint === `${i}-${k}` && (
          ...
          {shown.map((s, i) => (
            <div className="d-row" key={i} style={{ alignItems: "flex-start", padding: "9px 0", borderBottom: "1px solid var(--d-line)" }}>
              <button className="d-icon-btn" style={{ width: 32, height: 32, flexShrink: 0 }}
                onClick={() => playPhrase(s.he, s.audio)} aria-label={`Play ${s.he}`}>
                <Volume2 size={14} />
              </button>
              <div style={{ flex: 1 }}>
                <div className="d-prompt-he" dir="rtl" lang="he" style={{ fontSize: 21 }}>
                  {s.he.split(/(\s+)/).map((w, k) => {
                    ...
                    return (
                      <span key={k} style={{ position: "relative", display: "inline-block" }}>
                        <button type="button" className="d-hint" onClick={() => setHint(hint === `s${i}-${k}` ? null : `s${i}-${k}`)}>{w}</button>
                        {hint === `s${i}-${k}` && (
        ...
        <div style={{ marginTop: 14 }}>
          {doc.words.map((w, i) => (
            <div className="d-row" key={i} style={{ padding: "8px 2px", borderBottom: "1px solid var(--d-line)" }}>
              <button className="d-icon-btn" onClick={() => playPhrase(w.he, "")} aria-label={`Play ${w.he}`}><Volume2 size={16} /></button>
              <span className="d-prompt-he" dir="rtl" lang="he" style={{ fontSize: 22, flex: 1 }}>{w.he}</span>
